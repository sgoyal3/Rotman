...
class OrderBook:
    ...
    def update_trailing_data_RITC(self):
        ...
        if self.RITC_record_count > 0:
            ...
            # if no available data, just use the previous value
            try:
                vwap_ask /= volume_ask
            except ZeroDivisionError:
                vwap_ask = self.vwap_ask_arr[-1]

            try:
                vwap_bid /= volume_bid
            except ZeroDivisionError:
                vwap_bid = self.vwap_bid_arr[-1]
...
# manage our own book and size the tradeO
...
while get_tick() < 300:
    ...
    # update otherwise cancel multi times
    POSITION_MANAGER.updatePositionBook("RITC")
    if curr_position > -POSITION_MANAGER.ideal_position_limit:
        if VB < -0.3 or FB < -5000:
            POSITION_MANAGER.cancel_bad_orders(
